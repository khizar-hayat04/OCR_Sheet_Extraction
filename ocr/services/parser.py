...
def parse_azure_result(raw_response):
    ...
    2. Apply F/S value validation — two digits, ending in 0 or 5.
    3. Resolve ditto marks — propagate the last valid F/S value downward
       whenever a cell is detected as a ditto (blank or invalid single digit).
    ...
    if table:
        ...
        if cells:
            cells = _resolve_ditto_marks(cells)
            return cells
...
def _parse_table_cells(table, raw_response):
    ...
    for row_index in sorted(rows_by_index):
        ...
        # Precompute cleaned values for cells (keyed by id to allow dict/set membership)
        cell_clean = {id(c): _normalize_value(_clean_content(c.get('content','') or '')) for c in data_cells}
        ...
        for idx, (anchor_cx, gnum, ftype) in enumerate(anchor_x_list):
            ...
            if best_cell is not None and best_dist is not None and best_dist <= HEADER_ANCHOR_TOLERANCE:
                key = (gnum, ftype)
                accumulated[key] = cell_clean[id(best_cell)]
        ...
        remaining_cells = [c for c in data_cells if id(c) not in assigned_cells and cell_clean.get(id(c), '')]
...
# ---------------------------------------------------------------------------
# Ditto mark resolution — Two-pass approach
# ---------------------------------------------------------------------------

# A real, genuine F/S numeric value — at least 2 digits
_REAL_FS_VALUE = re.compile(r'^\d{2,}$')

# Obvious ditto indicators — things people write to mean "same as above"
_DITTO_INDICATORS = {
    '"',       # literal ditto mark
    "''",
    'do', 'ditto',
...
def _is_real_fs_value(value: str) -> bool:
    """
    True if value is a genuine 2+ digit number extracted by Azure.
    Does NOT include "x" — that's handled separately.
    Does NOT include single digits — those are ambiguous (real truncation? misread?).
    """
    if not value:
        return False
    return bool(_REAL_FS_VALUE.match(value))


def _is_ditto_mark(value: str) -> bool:
    """
    True if value is an explicit ditto indicator written by the person.
    Conservative — only obvious marks, not ambiguous short numbers.
    
    Does NOT treat "5", "8", etc. as dittos — they could be real truncated values.
    """
    if not value:
        return False
    norm = value.strip().lower()
    return norm in _DITTO_INDICATORS


def _resolve_ditto_marks(cells: list) -> list:
    """
    Two-pass ditto resolution:

    Pass 1 — Classify each cell as REAL, DITTO, KEEP_AS_IS, or EMPTY:
      REAL           — valid 2+ digit number or "x" → keep unchanged
      DITTO          — explicit ditto mark (", do, —) → needs resolution
      KEEP_AS_IS     — ambiguous value (1-digit, misread) → keep unchanged
      EMPTY          — blank cell → needs resolution

    Pass 2 — Resolve DITTO and EMPTY cells:
      For each DITTO/EMPTY cell at (row, group, field):
        Walk upward row by row through rows row-1, row-2, row-3, ...
        Find the first REAL value in the same (group, field) column.
        Copy that REAL value.
        If no REAL value found above → leave blank.

    This handles all edge cases:
      - Ditto chains of any length → always trace to the original real value
      - Multiple groups with different patterns → each column independent
      - Ambiguous values → preserved as extracted
      - "x" win markers → never overwritten, never cause chain breaks
    """
    cell_index: dict[tuple, OCRCell] = {}
    for cell in cells:
        key = (cell.row_number, cell.group_number, cell.field_type)
        cell_index[key] = cell

    # Pass 1: Classify each cell
    cell_class: dict[tuple, str] = {}  # REAL | DITTO | KEEP_AS_IS | EMPTY
    ...
    for cell in cells:
        ...
        if value == "":
            cell_class[key] = "EMPTY"
        elif value.lower() == "x":
            # "X" or "x" is always real (win marker in S column)
            cell_class[key] = "REAL"
        elif _is_real_fs_value(value):
            cell_class[key] = "REAL"
        elif _is_ditto_mark(value):
            cell_class[key] = "DITTO"
        else:
            # Ambiguous: single digit, or other short value
            # Could be real truncation (Azure misread "50" as "5")
            # Could be ditto (person wrote "1" for ditto line stroke)
            # Decision: KEEP_AS_IS — preserve what Azure extracted
            cell_class[key] = "KEEP_AS_IS"

    logger.debug(
        "Cell classification: %d REAL, %d DITTO, %d KEEP_AS_IS, %d EMPTY",
        sum(1 for c in cell_class.values() if c == "REAL"),
        sum(1 for c in cell_class.values() if c == "DITTO"),
        sum(1 for c in cell_class.values() if c == "KEEP_AS_IS"),
        sum(1 for c in cell_class.values() if c == "EMPTY"),
    )
    ...
    for row_num in range(1, 26):
        for group_num in range(1, 4):
            for field_type in ("N", "F", "S"):
                ...
                cls = cell_class.get(key)

                if cls == "REAL":
                    # Already correct — do nothing
                    pass
                elif cls == "KEEP_AS_IS":
                    # Ambiguous value — preserve as-is
                    preserved_count += 1
                    logger.debug(
                        "Kept ambiguous value row=%d g=%d f=%s: %r",
                        row_num, group_num, field_type, cell.value,
                    )
                elif cls == "EMPTY":
                    # Empty cells must remain empty — do not inherit
                    cell.value = ""
                    logger.debug("Left empty row=%d g=%d f=%s unchanged", row_num, group_num, field_type)
                elif cls == "DITTO":
                    # Explicit ditto marker — resolve by walking upward to find REAL value
                    found_value = None
                    found_from_row = None
                    for search_row in range(row_num - 1, 0, -1):
                        search_key = (search_row, group_num, field_type)
                        search_cell = cell_index.get(search_key)
                        if search_cell is None:
                            continue
                        search_class = cell_class.get(search_key)
                        if search_class == "REAL":
                            found_value = search_cell.value
                            found_from_row = search_row
                            break
                    ...
                    if found_value:
                        ...
                        resolved_count += 1
                        logger.debug(
                            "Ditto resolved row=%d g=%d f=%s: %r → %r (from row %d)",
                            row_num, group_num, field_type, old_value, found_value,
                            found_from_row,
                        )
                    else:
                        # No real value found above — stay empty
                        cell.value = ""
                        logger.debug(
                            "No real value above row=%d g=%d f=%s — cleared",
                            row_num, group_num, field_type,
                        )

    logger.info(
        "Ditto resolution complete: %d cells resolved, %d cells preserved as-is",
        resolved_count, preserved_count,
    )
    return cells
...
def _parse_with_word_fallback(raw_response):
    ...
    for row_number in range(1, 26):
        for col_index in range(1, 10):
            ...
            value = _normalize_value(" ".join(w["content"] for w in cell_words).strip())
